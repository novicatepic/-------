...
Унутар *buildroot* фолдера налазе се сви претходно креирани фајлови са вјежби и нови модификовани фајлови везани за *buildroot*.
...
Да би се добио бољи увид у експортоване сигнале UART1 периферије, на слици испод се може видјети гдје се на плочи налазе *RX* и *TX* пинови (за *UART1* у овом пројектном задатку остали пинови нису од интереса), као ни други експортовани сигнали на слици.
...
Такође, неопходно је измијенити садржај *boot-env.txt* фајла у складу са приложеним фајлом на овом репозиторијуму, а фајл треба да се налази у *board/terasic/de1soc_cyclone5* директоријуму уколико се прати првобитно подешавање *Buildroot-а* које је претходно споменуто.
...
Садржај фајла гдје је најбитније навести */dev/ttyS1* који је наведен и у *Buildroot* опцији *Where to look for GPSes* је приказан испод:
...
Након подешавања *gpsd* сервера на страни *De1-SoC* плоче, на развојној платформи (*Ubuntu 22.04 LTS*) можемо искористити неког од доступних *gpsd* клијената, у овом случају *xgps*.
...
Да би уопште било могуће да се повежемо са развојне платформе, неопходно је на циљној платформи унутар *lib/systemd/system/gpsd.socket* закометарисати (нпр. употребом *vi* едитора) помоћу *#* линије 
...
чиме се ослушкује само на *loopback* адреси и откоментарисати линије
...
Након тога, треба да се рестартује плоча командом *reboot* (сигурније рјешење) или евентуално извршити команду *systemctl restart gpsd*.
...
што је разлика у односу на 127.0.0.1:2947.
...
Један од примјера је представљен на сљедећој слици (разлог за лош квалитет јесте да је *Ubuntu* престао да ради у тренутку покушаја *screenshot-а*, а ово је један од ријетких тренутака када су се успјешно ухватили и сателити, па је усликано телефоном).
